...
import { Play, Pause, SkipBack, SkipForward, Droplet } from 'lucide-react';

interface WaterTimelineSliderProps {
    years: number[];
    currentYear: number;
    onYearChange: (year: number, previousYear?: number) => void;
    isPlaying?: boolean;
    onPlayToggle?: (playing: boolean) => void;
    onShowGif?: () => void;
}

export default function WaterTimelineSlider({
    years,
    currentYear,
    onYearChange,
    isPlaying = false,
    ...
    return (
        <div className="absolute top-20 left-4 z-[60]">
            {isMinimized ? (
                ...
                >
                    {localPlaying ? (
                        <Pause size={14} className="text-cyan-400 animate-pulse" />
                    ) : (
                ...
                <div className="bg-gray-900/95 backdrop-blur-md border border-gray-700/50 rounded-lg p-2 shadow-lg w-auto">
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            ...
                            className={`p-2 rounded shadow transition-all ${localPlaying
