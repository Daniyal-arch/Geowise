...
class NASAFIRMSService:
    ...
    def _parse_csv_response(self, csv_text: str, satellite: str) -> List[FireDetection]:
        ...
        CSV columns from NASA FIRMS:
        - latitude, longitude: Fire location
        - bright_ti4 (VIIRS) / brightness (MODIS): Brightness temperature (Kelvin)
        - scan, track: Pixel size (km)
        - acq_date, acq_time: Acquisition datetime (YYYY-MM-DD, HHMM)
        - satellite: Satellite identifier (e.g., 'N' for NOAA-20)
        ...
        for row in reader:
            try:
                # Parse date and time
                acq_date_str = row['acq_date']  # YYYY-MM-DD
                acq_time_str = row['acq_time']  # HHMM
                
                # Convert to datetime object
                acq_datetime = datetime.strptime(
                    f"{acq_date_str} {acq_time_str}", 
                    "%Y-%m-%d %H%M"
                )

                # Get brightness - VIIRS uses bright_ti4, MODIS uses brightness
                if 'bright_ti4' in row and row['bright_ti4']:
                    brightness = float(row['bright_ti4'])
                elif 'brightness' in row and row['brightness']:
                ...
                bright_ti5 = None
                if 'bright_ti5' in row and row['bright_ti5']:
                    try:
                ...
                track = 0.0
                if 'track' in row and row['track']:
                    try:
                        track = float(row['track'])
                    except ValueError:
                        track = 0.0

                # Create FireDetection with correct field names
                fire = FireDetection(
                    ...
                    scan=scan,
                    track=track,
                    acq_date=acq_datetime,  # ✅ DateTime object
                    acq_time=acq_time_str,   # ✅ String (HHMM)
                    satellite=row.get("satellite", ""),
                    instrument=row.get("instrument", ""),  # ✅ From CSV directly
        ...
        logger.info(f"✅ Successfully parsed {len(fires)} fire detections from NASA FIRMS CSV")
        return fires
    ...
    async def get_fires_by_bbox(
        ...
        try:
            ...
            if not csv_data.startswith("latitude"):
                raise NASAFIRMSAPIError(
                    f"NASA FIRMS API error: Unexpected response format: {csv_data[:100]}"
                )
            ...
            raise NASAFIRMSAPIError(
                f"NASA FIRMS API error: HTTP request failed: {str(e)}"
            ) from e
    ...
    def _get_country_bbox(self, country_iso: str) -> BoundingBox:
        ...
        try:
            ...
            raise NASAFIRMSAPIError(
                "NASA FIRMS API error: country-bounding-boxes library not installed. Run: pip install country-bounding-boxes"
            )
        except Exception as e:
